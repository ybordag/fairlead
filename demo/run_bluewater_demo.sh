...
LOG_DIR="${ROOT_DIR}/target/bluewater-demo"
...
echo
echo "3. Same-request retry: spark-a fails once, Fairlead retries spark-b"
set_mode "${SPARK_A_PORT}" "fail_once"
response="$(chat "spark-a" "retry-a")"
assert_source "${response}" "spark-b"

echo
echo "4. Circuit-open fallback: spark-a is healthy again, but its circuit is still open"
response="$(chat "spark-a" "fallback-a")"
assert_source "${response}" "spark-b"

echo
echo "5. Recovery: after cooldown, spark-a is tried again and succeeds"
sleep 3
response="$(chat "spark-a" "recovered-a")"
assert_source "${response}" "spark-a"

echo
echo "6. Metrics show the routing story"
...
echo
echo "7. Structured trace sample"
grep '"request completed"' "${FAIRLEAD_LOG}" | tail -n 3 || true

echo
echo "Bluewater demo completed successfully."
